...
class MainGame(object):
    ...
    def create_my_duck(self):
        MainGame.duck_p1 = MyDuck(420, 660, 0)
        global player_pos
        player_pos = [420, 660]
        ...
        while open_set:
            ...
            if heuristic(current, end) < self.rect.width:
                if self.will_collide_with_wall_in_next_move(direction):
                    ...
                    if direction in ['l', 'r']:
                        new_direction = 'u' if self.rect.top > MainGame.duck_p1.rect.top else 'd'
                        new_pos = (self.rect.x, self.rect.y + (60 if new_direction == 'd' else -60))
                    else:  # direction in ['u', 'd']
                        new_direction = 'l' if self.rect.left > MainGame.duck_p1.rect.left else 'r'
